...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    """
    interaction_adj, social_adj, similar_users_adj, _, similar_items_adj , _, _, _ = data_generator.get_norm_adj_mat()
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            # Cập nhật chuỗi hiển thị để bao gồm MRR và Hit Ratio (HR)
            pretrain_ret = 'pretrained model recall=[%s], precision=[%s], ndcg=[%s], ' \
                          'mrr=[%s], hr=[%s]' % \
    ...
    *********************************************************
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        *********************************************************
        ...
        # Bổ sung 2 độ đo MRR và Hit Ratio
        perf_str = 'Epoch %d: train==[%.5f=%.5f + %.5f + %.5f], recall=[%s], precision=[%s], ndcg=[%s], mrr=[%s], hit_ratio=[%s]' % \
                  (epoch, loss, mf_loss, emb_loss, reg_loss,
                    ', '.join(['%.5f' % r for r in ret['recall']]),
                    ', '.join(['%.5f' % r for r in ret['precision']]),
                    ', '.join(['%.5f' % r for r in ret['ndcg']]),
                    ', '.join(['%.5f' % r for r in ret['mrr']]),  # Thêm độ đo MRR
                    ', '.join(['%.5f' % r for r in ret['hit_ratio']]))  # Thêm độ đo Hit Ratio
        print(perf_str)
        ...
        *********************************************************
        ...
        # Thêm độ đo mới vào loggers nếu cần ghi nhận lại kết quả
        rec_loger.append(ret['recall'])
        pre_loger.append(ret['precision'])
        ndcg_loger.append(ret['ndcg'])
        hit_loger.append(ret['hit_ratio'])  # Thêm log hit_ratio
        mrr_loger.append(ret['mrr'])        # Thêm log MRR

        if args.verbose > 0:
          # Cập nhật chuỗi in kết quả trong vòng lặp epoch
          perf_str = 'Epoch %d: train==[%.5f=%.5f + %.5f + %.5f], recall=[%s], precision=[%s], ndcg=[%s], hit_ratio=[%s], mrr=[%s]' % \
                    (epoch, loss, mf_loss, emb_loss, reg_loss,
                      ', '.join(['%.5f' % r for r in ret['recall']]),
                      ', '.join(['%.5f' % r for r in ret['precision']]),
                      ', '.join(['%.5f' % r for r in ret['ndcg']]),
    ...
    # Cập nhật chuỗi in kết quả cuối cùng
    final_perf = "Best Iter=[%d]@[%.1f]\trecall=[%s], precision=[%s], ndcg=[%s], hit_ratio=[%s], mrr=[%s]" % \
                (idx, time() - t0, '\t'.join(['%.5f' % r for r in recs[idx]]),
                  '\t'.join(['%.5f' % r for r in pres[idx]]),
                  '\t'.join(['%.5f' % r for r in ndcgs[idx]]),
                  '\t'.join(['%.5f' % r for r in hit_loger[idx]]),
                  '\t'.join(['%.5f' % r for r in mrr_loger[idx]]))
    print(final_perf)
