...
#TODO: START ###################################################################################################
    ...
    # old code
    # def create_multimodal_similarity_matrix(self):
    ...
    #     # Lấy embeddings và chuyển về float32 
    #     text_embeddings = np.vstack(
    #         self.items_features['cleaned_combined_text'].apply(lambda x: get_bert_embeddings(x)).to_numpy()
    #     ).astype(np.float32)
    ...
    def create_multimodal_similarity_matrix(self, method="late_fusion", alpha=0.5, pca_components=256):
        ...
        if method == "late_fusion":
            ...
            image_tensor = torch.tensor(image_embeddings)

            # Tính attention weights
            attention_weights = F.softmax(
                torch.cat([text_tensor, image_tensor], dim=1), dim=1
            )

            # Kết hợp embeddings theo trọng số attention
            combined_embeddings = (
                attention_weights[:, :text_tensor.shape[1]] * text_tensor
                + attention_weights[:, text_tensor.shape[1]:] * image_tensor
            ).detach().numpy()
...
#TODO: START ###################################################################################################
        ...
        try:
            ...
            multimodal_similarity_adj_mat = self.create_multimodal_similarity_matrix(method="late_fusion", alpha=0.5, pca_components=384)
        ...
        return interaction_adj_mat_sym, social_adj_mat_sym, similar_users_adj_mat_sym, \
            ...
            full_bert_item_similarity_adj_mat_sym, multimodal_similarity_adj_mat_sym
