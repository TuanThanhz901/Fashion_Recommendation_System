...
def test(sess, model, users_to_test, drop_flag=False, train_set_flag=0):
    ...
    # Kiểm tra số phần tử trước khi reshape
    #print("Shape of all_result:", all_result.shape)

    final_result = np.mean(all_result, axis=0)  # mean

    # Xác định kích thước reshape dựa trên số phần tử thực tế
    expected_size = 6 * max_top
    if final_result.size != expected_size:
        raise ValueError(f"Expected size {expected_size}, but got {final_result.size}")

    final_result = np.reshape(final_result, newshape=[6, max_top])  # 7 metrics: precision, recall, etc.
    ...
    result['hit_ratio'] += final_result[4]  # HR
    result['mrr'] += final_result[5]        # MRR
    return result
